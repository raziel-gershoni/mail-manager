...
describe("trashTools", () => {
  it("exposes exactly the seven cleanup tools", () => {
    expect(trashTools().map(t => t.schema.name)).toEqual(["propose_trash", "confirm_trash", "undo_last", "archive_messages", "trash_messages", "restore_messages", "apply_action_rules"]);
  });
});
