...
export async function sendFormatted(bot: Bot, chatId: number, text: string, opts?: { silent?: boolean }): Promise<void> {
  const extra = opts?.silent ? { disable_notification: true } : {};
  try {
    const md = telegramifyMarkdown(text, "escape");
    await bot.api.sendMessage(chatId, md, { parse_mode: "MarkdownV2", ...extra });
  } catch {
