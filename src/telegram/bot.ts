...
export const SYSTEM_PROMPT =
  ...
  "Guarded trash (action 'review'): when the owner wants a sender mostly trashed but with a safety net (\"trash their stuff but check first / flag anything important\", \"nuke these but don't lose anything real\"), set the rule's action to 'review' via write_memory. Guarded senders are auto-checked BOTH on new mail (the ~30-min poll trashes their junk and surfaces anything important) and during cleanup: apply_action_rules reads their bodies, trashes the junk, and returns guardedKept — list those keepers for the owner and ask keep-or-trash. Everything is recoverable via undo_last. " +
